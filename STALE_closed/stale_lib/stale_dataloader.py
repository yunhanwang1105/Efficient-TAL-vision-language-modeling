...
###################### parse which annotation file to use #######################
import argparse
parser = argparse.ArgumentParser()
parser.add_argument('--anno_file', type=str, default='',
                      help='annotation file, eg. _20p')
args = parser.parse_args()

if args.anno_file:
    print("The currently used annotation file is: " + args.anno_file)
...
class STALEDataset(data.Dataset):
    def __init__(self, subset="train", mode="train", anno_file=''):
        # super().__init__()
        ...
        if anno_file:
            self.video_anno_path = './data/activitynet_annotations/' + anno_file
        ...
        print("init samples: ", len(self.subset_mask_list))
    ...
    def get_video_anno(self,video_infos,video_anno_path):
        ...
        # print(anno_database)
        video_dict = {}
        for video_name in anno_database.keys():
...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(STALEDataset(subset="train", anno_file=args.anno_file),
