...
class STALE(nn.Module):
    def __init__(self, temporal_scale=100):
    ...
    def crop_features(self, feature, mask):
        ...
        for i in range(mask.size(0)):
            ...
            top_mask = torch.where(mask[i,:] >= cls_thres, mask_fg[i,:], mask_bg[i,:]).cpu()
            top_loc = (top_mask==1).nonzero().squeeze().cpu()
    ...
    def text_features(self,vid_feat, mode):
        ...
        act_prompt = self.get_prompt(self.cl_names)
        texts = self.tokenizer(act_prompt, padding=True, return_tensors="pt").to('cpu')
